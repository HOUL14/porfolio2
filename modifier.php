...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifier_csrf_token($csrfToken, 'modifier_projet_' . $id)) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (!in_array($extension, $allowed, true)) {
            ...
            if (!move_uploaded_file($_FILES['image']['tmp_name'], $destination)) {
                ...
                if (!empty($imageActuelle)) {
                    $ancienFichier = __DIR__ . '/../../' . $imageActuelle;
    ...
    if (empty($erreurs)) {
        ...
        redirect('index.php?modifie=1');
    }
}

$csrfToken = generer_csrf_token('modifier_projet_' . $id);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
            ...
            <div class="form-group">
                ...
                <?php if ($imageActuelle): ?>
                    <div class="admin-image-preview">
                        <img src="../../<?= escape($imageActuelle) ?>" alt="Image du projet">
                    </div>
                <?php else: ?>
